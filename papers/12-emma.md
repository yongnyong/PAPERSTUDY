...
```mermaid
flowchart TB
    CAM["카메라 이미지(들)"] --> GEMINI
    NAV["내비게이션 지시,<br/>자차 상태(속도 등) — 텍스트로 인코딩"] --> GEMINI
    PROMPT["태스크별 프롬프트<br/>('궤적을 예측하라' / '객체를 검출하라' / '도로 그래프를 그려라')"] --> GEMINI

    GEMINI["Gemini 기반 MLLM<br/>(단일 모델, 공유 파라미터)"]

    GEMINI --> TRAJ["미래 궤적<br/>(좌표 시퀀스를 텍스트로 출력)"]
    GEMINI --> OBJ["3D 객체 위치/클래스<br/>(텍스트로 출력)"]
    GEMINI --> ROAD["도로 그래프 요소<br/>(차선, 정지선 등 텍스트로 출력)"]
```
